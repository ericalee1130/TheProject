...
public class CommonUtil {
	public static String getPageArea(ProductVo vo, String url, int curPage, int totPage, int pageRange){//무조건 public
		...
		for(int rp=startPage;rp<=endPage;rp++) {
			ret += "                            <li><a href='javascript:location.href=\""+url;
			if (vo.getPrimary_category() != null) {
				if(vo.getS_site() != null) { 
					ret += "?s_site="+vo.getS_site()+"&s_type="+vo.getS_type()+"&searchword="+vo.getSearchword()+
							"&primary_category="+vo.getPrimary_category()+"&orderCond="+vo.getOrderCond()+"&page="+rp+"\";'";
				} else {
					ret += "?primary_category="+vo.getPrimary_category()+"&orderCond="+vo.getOrderCond()+"&page="+rp+"\";'";
				}
			} else {
				if(vo.getS_site() != null) { 
					ret += "?s_site="+vo.getS_site()+"&s_type="+vo.getS_type()+"&searchword="+vo.getSearchword()+
							"&orderCond="+vo.getOrderCond()+"&page="+rp+"\";'";
				} else {
					ret += "?orderCond="+vo.getOrderCond()+"&page="+rp+"\";'";
				}
			}
